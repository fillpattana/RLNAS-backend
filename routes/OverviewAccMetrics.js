  ...
  try {
    ...
    for (let agentNum of agents) {
      ...
      for (let episodeNum of episodes) {
        ...
        const avgAccuracyResult = await pool.query(
          `SELECT ROUND(AVG(accuracy::numeric), 3) AS avg_accuracy 
        ...
        let avgAccuracy = avgAccuracyResult.rows[0].avg_accuracy;
        response[agentKey][episodeKey] =
          avgAccuracy !== null ? parseFloat(avgAccuracy) : null;
      }
    }
